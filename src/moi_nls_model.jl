...
Construct a `MathOptNLSModel` from a `JuMP` model and a container of JuMP expressions
`GenericAffExpr` (generated by @expression) and `NonlinearExpression` (generated by @NLexpression).
"""
function MathOptNLSModel(cmodel :: JuMP.Model, F; name :: String="Generic")
  ...
  ceval, Feval, nnlnequ = parser_nonlinear_expression(cmodel, nvar, F)

  nl_Fnnzj = nnlnequ == 0 ? 0 : sum(length(con.grad_sparsity) for con in Feval.constraints)
  nl_Fnnzh = nnlnequ == 0 ? 0 : sum(length(con.hess_I) for con in Feval.constraints)

  nl_cnnzj = nnln == 0 ? 0 : sum(length(con.grad_sparsity) for con in ceval.constraints)
  nl_cnnzh = (nnlnequ == 0 ? 0 : length(ceval.objective.hess_I)) + (nnln == 0 ? 0 : sum(length(con.hess_I) for con in ceval.constraints))
  ...
  return MathOptNLSModel(meta,
                         NLSMeta(nequ, nvar, nnzj=Fnnzj, nnzh=Fnnzh, lin=collect(1:nlinequ), nln=collect(nlinequ+1:nequ)),
                         Feval,
                         ceval,
                         lls,
                         linequ,
...
function NLPModels.residual!(nls :: MathOptNLSModel, x :: AbstractVector, Fx :: AbstractVector)
  ...
  if nls.nls_meta.nlin > 0
    coo_prod!(nls.linequ.jacobian.rows, nls.linequ.jacobian.cols, nls.linequ.jacobian.vals, x, view(Fx, nls.nls_meta.lin))
...
function NLPModels.jac_coord_residual!(nls :: MathOptNLSModel, x :: AbstractVector, vals :: AbstractVector)
  ...
  if nls.nls_meta.nnln > 0
    MOI.eval_constraint_jacobian(nls.Feval, view(vals, nls.linequ.nnzj+1:nls.nls_meta.nnzj), x)
  end
  return vals
end
...
function NLPModels.hprod_residual!(nls :: MathOptNLSModel, x :: AbstractVector, i :: Int, v :: AbstractVector, Hiv :: AbstractVector)
  ...
  if i ∈ nls.nls_meta.nln
    y = [j == i ? 1.0 : 0.0 for j = nls.nls_meta.nln]
...
function NLPModels.obj(nls :: MathOptNLSModel, x :: AbstractVector)
  ...
  if nls.nls_meta.nlin > 0
    obj += 0.5 * coo_sym_dot(nls.lls.hessian.rows, nls.lls.hessian.cols, nls.lls.hessian.vals, x, x) + dot(nls.lls.gradient, x) + nls.lls.constant
  end
  return obj
end
...
function NLPModels.hess_coord!(nls :: MathOptNLSModel, x :: AbstractVector, y :: AbstractVector, vals :: AbstractVector; obj_weight :: Float64=1.0)
  ...
  if (nls.nls_meta.nnln > 0) || (nls.meta.nnln > 0)
    MOI.eval_hessian_lagrangian(nls.ceval, view(vals, nls.lls.nnzh+1:nls.meta.nnzh), x, obj_weight, view(y, nls.meta.nln))
  end
  return vals
end
...
function NLPModels.hess_coord!(nls :: MathOptNLSModel, x :: AbstractVector, vals :: AbstractVector; obj_weight :: Float64=1.0)
  ...
  if nls.nls_meta.nnln > 0
    MOI.eval_hessian_lagrangian(nls.ceval, view(vals, nls.lls.nnzh+1:nls.meta.nnzh), x, obj_weight, zeros(nls.meta.nnln))
  else
