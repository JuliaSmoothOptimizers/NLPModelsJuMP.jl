...

{
    ...
    "category": "section",
    "text": "This package defines a NLPModels model using MathProgBase and JuMP.jl. This documentation is specific for this model. Please refer to the NLPModels documentation if in doubt."
},
...

{
    ...
    "title": "Tutorial",
    "category": "section",
    "text": "NLPModelsJuMP is a combination of NLPModels and JuMP, as the name implies. Sometimes it may be required to refer to the specific documentation, as we\'ll present here only the documention specific to NLPModelsJuMP.Pages = [\"tutorial.md\"]"
},
...

{
    ...
    "category": "section",
    "text": "The package OptimizationProblems provides a collection of problems defined in JuMP format, which can be converted to MathProgNLPModel.using OptimizationProblems # Defines a lot of JuMP models\n\nnlp = MathProgNLPModel(woods())\nx, fx, ngx, optimal, iter = steepest(nlp)\nprintln(\"fx = $fx\")\nprintln(\"ngx = $ngx\")\nprintln(\"optimal = $optimal\")\nprintln(\"iter = $iter\")Constrained problems can also be converted.using NLPModels, NLPModelsJuMP, JuMP\n\nmodel = Model()\nx0 = [-1.2; 1.0]\n@variable(model, x[i=1:2] >= 0.0, start=x0[i])\n@NLobjective(model, Min, (x[1] - 1)^2 + 100 * (x[2] - x[1]^2)^2)\n@constraint(model, x[1] + x[2] == 3.0)\n@NLconstraint(model, x[1] * x[2] >= 1.0)\n\nnlp = MathProgNLPModel(model)\n\nprintln(\"cx = $(cons(nlp, nlp.meta.x0))\")\nprintln(\"Jx = $(jac(nlp, nlp.meta.x0))\")"
},
...

{
    ...
    "category": "section",
    "text": "NLPtoMPBIn addition to creating NLPModels using JuMP, we might want to convert an NLPModel to a MathProgBase model to use the solvers available. For instanceusing Ipopt, NLPModels, NLPModelsJuMP, LinearAlgebra, JuMP, MathProgBase\n\nnlp = ADNLPModel(x -> dot(x, x), ones(2),\n                 c=x->[x[1] + 2 * x[2] - 1.0], lcon=[0.0], ucon=[0.0])\nmodel = NLPtoMPB(nlp, IpoptSolver())\n\nMathProgBase.optimize!(model)"
},
